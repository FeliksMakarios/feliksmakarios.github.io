...
{% for year in years %}{% assign year_pubs = pubs | where: "year", year %}<details class="year-group"><summary class="year-summary"><span class="year-label">{{ year }}</span><span class="year-count">{{ year_pubs.size }} {% if year_pubs.size == 1 %}publication{% else %}publications{% endif %}</span></summary><ul class="pub-list">{% for pub in year_pubs %}<li class="pub-item" data-area="{{ pub.area }}"><div class="pub-title">{% if pub.url and pub.url != "" %}<a href="{{ pub.url }}" target="_blank" rel="noopener external">{{ pub.title }}</a>{% else %}{{ pub.title }}{% endif %}</div><div class="pub-authors">{{ pub.authors | join: ", " }}</div><div class="pub-meta"><span class="venue-badge venue-{{ pub.venue_type }}">{{ pub.venue_type | capitalize }}</span><span class="pub-venue">{{ pub.venue }}</span></div></li>{% endfor %}</ul></details>{% endfor %}
</div>
